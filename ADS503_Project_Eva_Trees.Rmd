...
# Rsquared value of 0.066 isn't too great, but let's compare to other trees
```

### ctree
```{r}
# set.seed(123)
# Chol_ctree <- ctree(Chol_train_y ~., data = Chol_tr_X_imp_fin)
...
```{r}
set.seed(123)
# error in Chol_tr_X_imp_fin with DMDEDUC2_<9 not being found? will drop and train without until investigated further
Chol_tr_X_imp_rfdrop <- subset(Chol_tr_X_imp_fin, select = -c(DMD))

Chol_rf_model <- randomForest(Chol_train_y ~ ., data = Chol_tr_X_imp_rfdrop)
which.min(Chol_rf_model$mse) # this will tell the number of trees to get lowest MSE

Chol_rf <- train(x = Chol_tr_X_imp_fin, y = Chol_train_y, method = "rf", tuneLength = 10, trControl = Chol_control)
Chol_rf
plot(Chol_rf)
